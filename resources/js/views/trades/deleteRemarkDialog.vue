<template>
    ...
    >
    <v-card>
        <v-card-title class="headline">DELETE this Trade?</v-card-title> 
        <v-card-text>             
            <v-container>
            <v-row>   
                <v-col cols="12">
                    Id: {{ trade.id }} <br/>
                    Name: {{trade.name}} <br />
                    Description: {{trade.description}} <br />
                </v-col>
                <v-col cols="12">
                    <v-textarea 
                        auto-grow 
...
export default {
    ...
    props: {
        trade: { type: Object, default: function () {return false}},
    ...
    methods : {
        ...
        deleteTrade() {
            ...
            if (this.$refs.saveform.validate()) {
                ...
                this.loading = true
                this.$store.dispatch('deleteTrade',{id: this.trade.id, delete_remark: this.delete_remark})
                .then(response => {
                    this.loading = false;
                    this.close()
